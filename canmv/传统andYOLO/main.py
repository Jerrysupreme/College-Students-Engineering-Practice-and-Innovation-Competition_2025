...
thresholds = [(0, 100, 15, 127, 1, 127), # 红色阈值 (30, 100, 15, 127, 15, 127)#(0, 100, 15, 127, 1, 127)
...
def detect_start():
    ...
    if det_boxes2:
        ...
        pl.osd_img.clear()
    return 0,0,0,0
def yolo_detect(cs = 0,cl = 0,start = 0):#cs只能取0或3，表红色和蓝色,cl表球真实色彩取0,1,2,3
...
def color_detect(img,cs = 0,cl = 0,start = 0):#cs只能取0或3，表红色和蓝色,cl表球真实色彩取0,1,2,3
    global precs,color,send_flag
    send_flag = 0
    p = {}
    q = {}
    ...
    if blobs:
        for b in blobs: #画矩形、箭头和字符表示
            ...
            if b.w() < ball_max and b.h() < ball_max:
                p[b.cy()] = b.cx()
            if b.w()/b.h() > 5 or b.w() > ball_max or b.h() > ball_max:
                q[b.cy()] = b.cx()
                qarea[b.cy()] = b.area()
        if len(p):
           by, bx = min(p.items(), key=lambda item: (item[1] - 420)**2 + (item[0] - 480)**2)
    ...
    if cl == 1 or cl == 2:
        ...
        if len(p):
            by, bx = min(p.items(), key=lambda item: (item[1] - 420)**2 + (item[0] - 480)**2)
    send_data(bx,by,qx,qy,0,0,ins1 = ins,ins2 = send_flag)
def read_uart():
    while True:
        text = uart.read(1)
#        print("thread on")
...
if __name__ == "__main__":
    ...
    kmodel_path2="/sdcard/app/chufaqv2.kmodel" #不规则
    ...
    try:
        while True:
            ...
            with ScopedTiming("total",1):
                ...
                #yolo_detect(cs = color,cl = mode,start=start)
                color_detect(img,cs = color,cl = mode,start=start)
