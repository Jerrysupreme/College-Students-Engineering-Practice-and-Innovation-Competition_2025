...
def detect(cs = 0):
    ...
    qx = 0
    qy = 0
    blobs = img.find_blobs([thresholds[cs]],area_threshold=200,pixels_threshold=10) # 0,1,2分别表示红，绿，蓝色。
    if blobs:
        for b in blobs: #画矩形、箭头和字符表示
            ...
            area_str = f"area:{b.area()},长宽:{b.w()},{b.h(),ratio:{b.area()/384000}}"
...
while True:
    ...
    btn()
    if is_recording:
        img.save(f"/data/img_{number}.jpg")  # 保存图片
        number += 1
    img.draw_string_advanced(0, 0, 30, 'FPS: '+str("%.3f"%(clock.fps()))+f"total_{number}", color = (255, 255, 255))
