...
def detect(cs = 0,cl = 0):#cs只能取0或3，表红色和蓝色,cl表球真实色彩取0,1,2,3
    ...
    if det_boxes:
        ...
        for b in det_boxes: #画矩形、箭头和字符表示
            ...
            x1, y1, x2, y2 = map(int, b[2:6])
            x1=int(x1 * yolo.display_size[0] // yolo.rgb888p_size[0])
            y1=int(y1 * yolo.display_size[1] // yolo.rgb888p_size[1])
            x2=int(x2 * yolo.display_size[0] // yolo.rgb888p_size[0])
            y2=int(y2 * yolo.display_size[1] // yolo.rgb888p_size[1])
        ...
        if len(qr):
            qyr = max(qr.keys())
            qxr = qr[qyr]
            if(qyr>QY_MIN and cs == 0):
                ins = 1
        if len(qb):
            qyb = max(qb.keys())
            qxb = qb[qyb]
            if(qyb>QY_MIN and cs == 3):
                ins = 1
    else:
        pl.osd_img.clear()
    if cl == 1 or cl == 2:
        ...
        if len(p):
            by = max(p.keys())
            bx = p[by]
    send_data(bx,by,qxr,qyr,qxb,qyb,ins1 = ins,ins2 = send_flag)
# def read_uart():
#     while True:
#         text = uart.read(1)
#         if text:
...
if __name__ == "__main__":
    ...
    if display_mode=="hdmi":
        ...
        display_size=[800,480]
    kmodel_path="/sdcard/app/best_AnchorBaseDet_can2_5_n_20250415003323.kmodel"
   #best_AnchorBaseDet_can2_5_n_20250321000750.kmodel
   #best_AnchorBaseDet_can2_5_n_20250325172210.kmodel
   #best_AnchorBaseDet_can2_5_n_20250415003323.kmodel 广角模型
    labels=["红方安全区","黑球","黄球","蓝球","红球","黑方安全区"]
    ...
    pl.create()
    uart = hardware_init()
    yolo = YOLODetector(kmodel_path,labels,model_input_size=[640,640],anchors=anchors,rgb888p_size=[1280,720],display_size=display_size,debug_mode=0)
    yolo.config_preprocess()
    # _thread.start_new_thread(read_uart,("1",)) #开启线程1,读取uart
    try:
        while True:
            os.exitpoint()
            with ScopedTiming("total",1):
                img=pl.get_frame()
                det_boxes = yolo.run(img)
                text = uart.read(1)
                if text:
                    handle_category(text)
