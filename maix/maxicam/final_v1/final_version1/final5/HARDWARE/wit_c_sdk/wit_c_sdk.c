...
void WitSerialDataIn(uint8_t ucData)
{
    ...
    if(s_ucWitDataBuff[0] != 0x55)//不符合数据帧
    {
        s_uiWitDataCnt--;
    ...
    {
        ucSum = __CaliSum(s_ucWitDataBuff, 10);//计算前十字节的校验和
        if(ucSum != s_ucWitDataBuff[10])//校验失败就移除缓存
        {
            s_uiWitDataCnt--;
            memcpy(s_ucWitDataBuff, &s_ucWitDataBuff[1], s_uiWitDataCnt);
            return ;
...
*/
	if(WitWriteReg(KEY1, KEY_UNLOCK) != WIT_HAL_OK)	    return  WIT_HAL_ERROR;// unlock reg
...
{
	if(WitWriteReg(KEY1, KEY_UNLOCK) != WIT_HAL_OK)	return  WIT_HAL_ERROR;
	if(s_uiProtoclo == WIT_PROTOCOL_MODBUS)	p_WitDelaymsFunc(20);
	else if(s_uiProtoclo == WIT_PROTOCOL_NORMAL) p_WitDelaymsFunc(1);
	else ;
...
int32_t WHT101_ANGLEZCali(void)                      //101z轴归0
{
	if(WitWriteReg(KEY1, KEY_UNLOCK) != WIT_HAL_OK)	    return  WIT_HAL_ERROR;
	if(s_uiProtoclo == WIT_PROTOCOL_MODBUS)	p_WitDelaymsFunc(20);
	else if(s_uiProtoclo == WIT_PROTOCOL_NORMAL) p_WitDelaymsFunc(1);
