...
void show_task(void *pvParameters)
{
   ...
   {	
		...
		vTaskDelayUntil(&lastWakeTime, F2T(RATE_100_HZ));//This task runs at 10Hz //此任务以10Hz的频率运行
...
//					OLED_ShowString(60,0,"GZ");
					...
					if( rotation_Kp<0)  OLED_ShowString(25,0,"-"),oled_showfloat(- rotation_Kp,30,0,1,3);
					else       
						OLED_ShowString(25,0,"+"),oled_showfloat(rotation_Kp,30,0,  1,3);	
					if( fAngle1<0)  OLED_ShowString(80,0,"-"),OLED_ShowNumber(90,0,-fAngle1,5,12);
					else       
						OLED_ShowString(80,0,"+"),OLED_ShowNumber(90,0, fAngle1,5,12);		
				}
			 }
...
//	
	...
	//															OLED_ShowNumber(75,30, Remoter_Ch4,5,12);
			 ...
			 //Line 5 of the display displays the content//
			 //显示屏第5行显示内容//
			if( rotation_Kd<0)  OLED_ShowString(5,50,"-"),oled_showfloat(- rotation_Kd,10,50,1,3);
			else       
				OLED_ShowString(5,50,"+"),oled_showfloat( rotation_Kd,10,50,1,3);	
			if( rotation_Ki<0)  OLED_ShowString(55,50,"-"),oled_showfloat(- rotation_Ki,60,50,1,3);
			else       
				OLED_ShowString(55,50,"+"),oled_showfloat( rotation_Ki,60,50, 1,3);	
